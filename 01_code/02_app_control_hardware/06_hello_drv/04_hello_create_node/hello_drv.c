...
static int hello_probe(struct platform_device *pdev)
{
	...
	major = register_chrdev(0, "100ask_hello_drv", &hello_fops);
	printk("register_chrdev major = %d\n", major);

	hello_dev_class = class_create(THIS_MODULE, "hello");
	if (IS_ERR(hello_dev_class))
		return -EINVAL;
	device_create(hello_dev_class, NULL, MKDEV(major, 0), NULL, "abcxyz");
...
{
	device_destroy(hello_dev_class, MKDEV(major, 0));
	class_destroy(hello_dev_class);
	unregister_chrdev(major, "100ask_hello_drv");
	return 0;
}
